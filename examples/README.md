...
## Demo Scripts of Interaction with xdcBridge Protocol

This repository demonstrates how to interact with xdcBridge infrastructure in order to send arbitrary data and liquidity between any blockchains supported by the protocol.
...
xdcBridge provides an explorer that allows tracking the status of all cross-chain transactions that pass through the protocol. Just put in txId in the search bar in order to see your transaction details
...
In order to have the transaction executed in the target chain, it should be claimed by passing all parameters of the transaction alongside signatures of submissionId from all xdcBridge validators. 
...
Same transaction can be found by txid in [xdcBridge explorer](
...
## Cross-chain swap
Set xdcBridgeGATE_ADDRESS, SENDER_PRIVATE_KEY, ROUTER_ADDRESS in .env file
