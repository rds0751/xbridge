...
async function getTokenAddressOnToChain(
    ...
    const nativeTokenInfo = await xdcBridgeGateFrom.methods.getNativeInfo(tokenAddressOnFromChain).call();
    ...
    const xdcBridgeId = isNativeToken
        ?  await getXbridgeId(xdcBridgeGateFrom, xdcBridgeGateTo, fromChainId, tokenAddressOnFromChain)
        :  await getXbridgeId(xdcBridgeGateFrom, xdcBridgeGateTo, nativeChainId, nativeTokenInfo.nativeAddress)

    const xdcBridgeInfo = await xdcBridgeGateTo.methods.getXbridge(xdcBridgeId).call();

    logger.info(`Sending ${isNativeToken ? '' : 'not '}native token`);
    logger.info(`nativeTokenInfo`, nativeTokenInfo);
    logger.info('xdcBridgeId of sending token', xdcBridgeId);
    logger.info('xdcBridgeInfo',xdcBridgeInfo);

    if (!xdcBridgeInfo.exist) {
    ...
    return isNativeToken ? xdcBridgeInfo.tokenAddress : nativeTokenInfo.nativeAddress;
}
