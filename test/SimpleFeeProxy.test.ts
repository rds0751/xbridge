...
let xdcBridgeGateMock: XDCBridgeGateMock;
...
beforeEach(async () => {
    ...
    xdcBridgeGateMock = await deployMockContract(deployer, XDCBridgeGateJson.abi) as XDCBridgeGateMock;
    await xdcBridgeGateMock.mock.getNativeInfo
        .withArgs(erc20Mock.address)
        .returns(CHAIN_ID, NATIVE_ADDRESS);
    await xdcBridgeGateMock.mock.withdrawFee.returns();
...
describe('withdrawFee', () => {
    ...
    it('calls withdrawFee with correct id', async () => {
        ...
        await expectStubFunctionIsCalledWith(
            xdcBridgeGateMock.mock.withdrawFee,
            [correctId],
...
describe('onlyAdmin functions', () => {
    ...
    const expectProtectedFunctionsCalledByAccountTo = async (
        ...
        const protectedFunctions = [
            ...
            async () => {await simpleFeeProxyConnected.setXbridgeGate(xdcBridgeGateMock.address)},
