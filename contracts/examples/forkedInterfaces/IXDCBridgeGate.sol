/**
It's a fork of IXDCBridgeGate.sol with callProxy getter added
...
interface IXDCBridgeGate {
    ...
    /// @dev This method is used for the transfer of assets [from the native chain](https://docs.xbridge.finance/the-core-protocol/transfers#transfer-from-native-chain).
    ...
    /// @dev Is used for transfers [into the native chain](https://docs.xbridge.finance/the-core-protocol/transfers#transfer-from-secondary-chain-to-native-chain)
    ...
    /// @param _xbridgeId Asset identifier.
    function withdrawFee(bytes32 _xbridgeId) external;


    /// @dev Returns address of the proxy to execute user's calls.
    function callProxy() external view returns (address);

    /// @dev Returns asset fixed fee value for specified xbridge and chainId.
    /// @param _xbridgeId Asset identifier.
    ...
    event WithdrawnFee(bytes32 xbridgeId, uint256 fee);
