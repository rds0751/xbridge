...
/// @dev Example contract to show how to send a simple message to another chain using xdcBridgeGate
contract Incrementor is BridgeAppBase {
    uint256 public claimedTimes;
    using Flags for uint256;

    function initialize(IXDCBridgeGate _xdcBridgeGate) external initializer {
        __BridgeAppBase_init(_xdcBridgeGate);
        claimedTimes = 0;
    }
    ...
    function send(
        ...
        xdcBridgeGate.send{value: msg.value}(
            address(0),
            msg.value,
            _chainIdTo,
