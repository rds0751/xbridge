...
contract DefiController is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    IXDCBridgeGate public xdcBridgeGate;
    ...
    function depositToStrategy(uint256 _amount, address _strategy) internal {
        ...
        uint256 avaliableReserves = xdcBridgeGate.getDefiAvaliableReserves(strategy.stakeToken);
        ...
        xdcBridgeGate.requestReserves(strategy.stakeToken, _amount);
    ...
    function withdrawFromStrategy(uint256 _amount, address _strategy) internal {
        ...
        IERC20Upgradeable(strategy.stakeToken).safeApprove(address(xdcBridgeGate), 0);
        IERC20Upgradeable(strategy.stakeToken).safeApprove(address(xdcBridgeGate), _amount);

        // TODO: get rewards from strategy

        // Return tokens to Gate
        xdcBridgeGate.returnReserves(strategy.stakeToken, _amount);
    ...
    function setXDCBridgeGate(IXDCBridgeGate _xdcBridgeGate) external onlyAdmin {
        xdcBridgeGate = _xdcBridgeGate;
    }
