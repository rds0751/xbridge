...
contract CallProxy is Initializable, AccessControlUpgradeable, ICallProxy {
    ...
    bytes32 public constant xdcBridge_GATE_ROLE = keccak256("xdcBridge_GATE_ROLE");
    ...
    modifier onlyGateRole() {
        if (!hasRole(xdcBridge_GATE_ROLE, msg.sender)) revert XDCBridgeGateBadRole();
        _;
    }
    ...
    // we need to accept ETH from xdcBridgeGate
